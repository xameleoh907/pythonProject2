...
# функция фозвращает True если найдены несоответсвия адреса и False если несоответствий не найдено
    ...
    for i in check_list:
        if mail.endswith(i) and '@' in mail: # проверка одновременного наличия @ и домена почны
            return False
    else:
        return True
...
send_email('Это сообщение для проверки связи', 'vasyok.com@gmail')
...
send_email('Это сообщение для проверки связи', 'vasyok1337@gmail.com', sender='uversity.help@gmail.com')
send_email('Пожалуйста, исправьте задание', 'urban.student@mail.ru', sender='urban.teacher@mail.uk')
send_email('Пожалуйста, исправьте задание', 'ban.student@mail.ru', sender='urban.teacher@mail.uk')
